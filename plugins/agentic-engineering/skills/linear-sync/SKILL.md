...
(thin markdown)          (agentic-plugin linear)          (GraphQL + API key)

/linear:sync    ──────>  agentic-plugin linear sync   <─────> Linear (push + pull)
/linear:status  ──────>  agentic-plugin linear status ──────> Linear (read)
/linear:import  ──────>  agentic-plugin linear import <────── Linear (pull)
/linear:pull    ──────>  agentic-plugin linear pull   <────── Linear (pull)
```
...
Set in `agentic-engineering.local.md` frontmatter or environment variables:
...
If only one team exists in the workspace, it auto-detects. Run `agentic-plugin linear config` to verify.
...
Triggered by lifecycle commands and `agentic-plugin linear sync`:
...
Triggered by `agentic-plugin linear sync` and `agentic-plugin linear pull`:
...
agentic-plugin linear sync --todos-dir ./todos [--dry-run]

# Push local changes to Linear
agentic-plugin linear push --todos-dir ./todos [--file <path>]

# Pull Linear changes into local files
agentic-plugin linear pull --todos-dir ./todos

# Show sync status dashboard
agentic-plugin linear status --todos-dir ./todos

# Import a specific Linear issue as a todo file
agentic-plugin linear import ENG-123 --todos-dir ./todos

# Create a Linear issue from a file
agentic-plugin linear create <file-path> [--parent ENG-140]

# Cancel a Linear issue
agentic-plugin linear cancel ENG-123 [--comment "reason"]

# Show resolved configuration
agentic-plugin linear config
```
...
|---|---|
| `/workflows:review` creates todos | `agentic-plugin linear create <file>` for each |
| `/triage` approves todo | `agentic-plugin linear push --file <path>` |
| `/triage` skips todo | `agentic-plugin linear cancel <linear-id>` |
| `/resolve_todo_parallel` starts | `agentic-plugin linear push --file <path>` |
| `/resolve_todo_parallel` completes | `agentic-plugin linear push --file <path>` |
| `/workflows:plan` creates plan | `agentic-plugin linear create <plan-path>` |
| `/workflows:work` starts | `agentic-plugin linear pull` then push |
...
agentic-plugin linear create docs/plans/2026-02-24-feat-api-rate-limiting-plan.md

# Create sub-issue linked to parent
agentic-plugin linear create todos/042-ready-p1-rate-limiter.md --parent ENG-140
```
