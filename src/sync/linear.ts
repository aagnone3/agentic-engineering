  ...
  for (const filePath of mdFiles) {
    ...
    todos.push({
      ...
      frontmatter: {
        ...
        linear_synced_at: data.linear_synced_at
          ? String(data.linear_synced_at)
          : undefined,
      },
      body,
    });
...
export function fileStatusToLinearState(
  ...
  return states.find((s) => s.name.toLowerCase() === targetName.toLowerCase());
}
...
export function filePriorityToLinear(
  priority: string,
  config: LinearConfig
): number {
...
export function linearPriorityToFile(
  priority: number,
  _config: LinearConfig
): string {
...
export function renameTodoFile(
  ...
  return path.join(
    dir,
    `${issueId}-${newStatus}-${newPriority}-${description}.md`
  );
}
...
export async function updateTodoFrontmatter(
  ...
  if (
    updates.linear_id !== undefined ||
    updates.linear_synced_at !== undefined
  ) {
    merged.linear_synced_at =
...
export async function updatePlanFrontmatter(
  ...
  const merged = { ...data, ...updates };
  if (
    updates.linear_issue !== undefined ||
    updates.linear_synced_at !== undefined
  ) {
    merged.linear_synced_at =
      updates.linear_synced_at ?? new Date().toISOString();
...
export async function pushCreate(
  ...
  const state = fileStatusToLinearState(
...
export async function pullCommentsFromIssue(
  ...
  for (const comment of newComments) {
    ...
    const author =
      comment.user?.displayName ?? comment.user?.name ?? "Linear User";
    appendText += `\n### ${date} - Linear Comment\n\n`;
...
export async function pullNewIssues(
  ...
  for (const issue of newIssues) {
    ...
    const status =
      linearStateToFileStatus(issue.state.name, config) ?? "pending";
...
export async function importSingleIssue(
  ...
  const body = `# ${issue.title}\n\n## Problem Statement\n\n${
    ...
    new Date().toISOString().split("T")[0]
  } - Imported from Linear\n\n**By:** agentic-plugin linear import\n\n**Actions:**\n- Imported ${
    issue.identifier
...
export async function syncAll(
  ...
  for (const todo of todos) {
    if (todo.frontmatter.linear_id) {
      ...
      try {
        ...
        const issue = await api.getIssueByIdentifier(
        ...
        if (linearChanged && fileChanged) {
          ...
          const linearStatus =
            linearStateToFileStatus(issue.state.name, config) ??
            issue.state.name;
          ...
          if (!opts.dryRun) {
            const pullResult = await pullStateFromIssue(
              issue,
              todo.path,
              config
            );
        ...
        result.errors.push(
          `Error syncing ${path.basename(todo.path)}: ${
      ...
      if (!opts.dryRun) {
        try {
          ...
          console.log(
            `  Created ${identifier} for ${path.basename(todo.path)}`
          );
          result.pushed.created++;
        } catch (err) {
          result.errors.push(
            `Error creating Linear issue for ${path.basename(todo.path)}: ${
              err instanceof Error ? err.message : String(err)
            }`
          );
        }
        ...
        console.log(
          `  Would create Linear issue for ${path.basename(todo.path)}`
        );
        result.pushed.created++;
      }
    }
  ...
  if (!opts.dryRun) {
    try {
      const newTodos = await pullNewIssues(
        config,
        todosDir,
        existingLinearIds,
        maxIssueId
      ...
      for (const todo of newTodos) {
        console.log(
          `  Pulled ${todo.frontmatter.linear_id} → ${path.basename(todo.path)}`
        );
      }
    } catch (err) {
      result.errors.push(
        `Error pulling new issues: ${
          err instanceof Error ? err.message : String(err)
        }`
      );
    }
  ...
  for (const todo of todos) {
    ...
    if (todo.frontmatter.linear_id) {
      try {
        ...
        const expectedStatus = linearStateToFileStatus(
          issue.state.name,
          config
        );
        ...
        if (
          expectedStatus !== todo.frontmatter.status ||
          expectedPriority !== todo.frontmatter.priority
        ) {
          entry.inSync = false;
...
export async function resolveConfig(
  ...
  // Try to read from agentic-engineering.local.md
  if (projectRoot) {
    const localPath = path.join(projectRoot, "agentic-engineering.local.md");
  ...
  if (!teamId || !teamKey) {
    ...
    if (teams.length === 1) {
      ...
      console.log(
        "Multiple teams found. Set LINEAR_TEAM_ID or linear_team_id in agentic-engineering.local.md:"
      );
