...
describe("convertClaudeToPi", () => {
  test("converts commands, skills, extensions, and MCPorter config", async () => {
    ...
    expect(
      bundle.prompts.some((prompt) => prompt.name === "workflows-review")
    ).toBe(true);
    expect(bundle.prompts.some((prompt) => prompt.name === "plan_review")).toBe(
      true
    );

    // Commands with disable-model-invocation are excluded
    expect(bundle.prompts.some((prompt) => prompt.name === "deploy-docs")).toBe(
      false
    );

    const workflowsReview = bundle.prompts.find(
      (prompt) => prompt.name === "workflows-review"
    );
    expect(workflowsReview).toBeDefined();
    const parsedPrompt = parseFrontmatter(workflowsReview!.content);
    expect(parsedPrompt.data.description).toBe(
    ...
    const compatExtension = bundle.extensions.find(
      (extension) => extension.name === "agentic-engineering-compat.ts"
    );
    ...
    expect(bundle.mcporterConfig?.mcpServers.context7?.baseUrl).toBe(
      "https://mcp.context7.com/mcp"
    );
    expect(bundle.mcporterConfig?.mcpServers["local-tooling"]?.command).toBe(
      "echo"
    );
  });
  ...
  test("transforms Task calls, AskUserQuestion, slash commands, and todo tool references", () => {
    ...
    expect(parsedPrompt.body).toContain(
