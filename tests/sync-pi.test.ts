...
describe("syncToPi", () => {
  test("symlinks skills and writes MCPorter config", async () => {
    ...
    const mcporterConfig = JSON.parse(
      await fs.readFile(mcporterPath, "utf8")
    ) as {
      mcpServers: Record<string, { baseUrl?: string; command?: string }>;
    };

    expect(mcporterConfig.mcpServers.context7?.baseUrl).toBe(
  ...
  test("merges existing MCPorter config", async () => {
    ...
    await fs.writeFile(
      mcporterPath,
      JSON.stringify(
        { mcpServers: { existing: { baseUrl: "https://example.com/mcp" } } },
        null,
        2
      )
    );
    ...
    expect(merged.mcpServers.context7?.baseUrl).toBe(
      "https://mcp.context7.com/mcp"
    );
  });
});
